...
def save_to_csv(candidates, csv_path):
    fieldnames = ["name", "email", "phone", "linkedin", "github", "internship", "notes", "attachments"]
...
# ------------------ Core Scraper ------------------
def fetch_and_save_emails(imap_server, email_user, email_pass, folder="INBOX", attachment_folder="attachements", csv_path="candidates.csv", code_map=None):
    try:
    ...
    for msg_id in messages[0].split():
        ...
        for response_part in msg_data:
            if isinstance(response_part, tuple):
                ...
                candidate_data["attachments"] = ", ".join(saved_files)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Internship Email Scraper")
    ...
    parser.add_argument('--csv-path', type=str, default='candidates.csv', help='CSV file to save candidates')
    parser.add_argument('--code-map', type=str, default='', help='JSON string for internship code mapping')
    args = parser.parse_args()
    code_map = json.loads(args.code_map) if args.code_map else None
